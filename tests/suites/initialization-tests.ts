...
import { createRatioMint, getTokenBalance, getUsers, mapUsers, mintRatioTo } from '../utils';

export default function suite() {
  describe('mints and users', function () {
    it('can create mint', async function () {
      expect((await createRatioMint(this.connection, this.payer, this.publicKey)).toString()).to.equal(
        this.mint.toString()
      );
    });
    ...
    it('can create main user and fund mint', async function () {
      ...
      // fund user
      await mintRatioTo(this, this.accounts.user, this.constants.mintSupply);
